...
API_KEY = "5aecc92f76124bc8bce4b594066bac08"
API_SECRET = "cb02bcbbafe64d0782211e99257510da"
units_bought = 0
currency = sys.argv[1]
...
def get_url(api_type, **params):
    ...
    if params["action"] == "buy" or params["action"] == "sell":
        ...
        url = url % (params["uuid"])
    elif params["action"] == "open_orders" or params["action"] == "market_ticker" or params[
        "action"] == "market_day_summary" or params["action"] == "last_trades" or params[
        "action"] == "market_orders_history":
...
def call_api(url):
    try:
        ...
        print
        Fore.YELLOW, "Fetching Market Summary..."
        # print (url + "\n" + parsed_body)
        print
        Fore.GREEN + "Success" if parsed_body["success"] is True else Fore.RED + "Failed"
        if parsed_body["success"] is True:
            return parsed_body["result"]
    except:
        print
        "Call api failed: ", sys.exc_info()[0]
...
def call_secret_api(url):
    ...
    print
    Fore.YELLOW, "Calling API..."
    # print response
    parsed_body = response.json()
    # p [url, parsed_body]
    print
    Fore.GREEN + "Success" if parsed_body["success"] is True else Fore.RED + "Failed: " + parsed_body["message"]
    if parsed_body["success"] is True:
        return parsed_body["result"]
        ...
        if market["IsActive"] and base_currency == "BTC" or base_currency == "ETH":
            ...
            if balance_details["Available"] and balance_details["Available"] > 0.0:  # purchased coins
                ...
                if net_value > 0:  # buys are more, we need to get more than this net value by selling available coins
                    ...
                    sell_limit_url = get_url(URI_market, action="sell", market=market_name,
                                             quantity=balance_details["Available"], rate=sell_price)
                    order_placed = call_secret_api(sell_limit_url)
                    print
                    order_placed, "for #%s at #%s" % (market_name, sell_price)
...
def buy_chunk(last_price, market_name, percent_increase, chunk):
    ...
    print
    Fore.LIGHTYELLOW_EX + "Purchasing coin..." + Fore.WHITE
    print
    {"api_type": "market", "action": "buy", "market": market_name, "quantity": quantity, "rate": unit_price}
    order = call_secret_api(buy_limit_url)
    print
    Fore.GREEN + "Success" if order is not None and order["uuid"] is not None else Fore.RED + "Fail"
    cnt = 1
    while cnt <= 3 and order is not None and order["uuid"] is not None:  # retry
        print
        Fore.YELLOW + "Retry #{cnt}: Purchasing coin..." + Fore.WHITE
...
def buy_bot(percent_increase=0.05, chunk=0.006, prepump_buffer=0.5):
    ...
    total_spent = 0.0
    print
    {"low_24_hr": low_24_hr, "last_price": last_price, "ask_price": ask_price, "volume": volume}
    if volume < 100 and last_price < (
        1.0 + prepump_buffer) * low_24_hr:  # last_price is smaller than 50% increase since yerterday
        print
        Fore.BLUE + "Coin is not prepumped" + Fore.WHITE
        order = buy_chunk(last_price, market_name, percent_increase, chunk)
        print
        order, "Units Bought : #{%s}" % (units_bought)
...
def sell_bot(percent_decrease=0.1):
    ...
    if balance_details and balance_details["Available"] and balance_details["Available"] > 0.0:
        ...
        sell_limit_url = get_url(URI_market, action="sell", market=market_name, quantity=balance_details["Available"],
                                 rate=sell_price)
        print
        Fore.YELLOW, "Selling coin..."
        print
        {api_type: "market", action: "sell", market: market_name, quantity: balance_details["Available"],
         rate: sell_price}
        order_placed = call_secret_api(sell_limit_url)
        ...
        while cnt <= 3 and order_placed and order_placed["uuid"] is None:  # retry
            print
            Fore.YELLOW, "Retry #{cnt} : Selling coin..."
            time.sleep(.5)  # half second
            order_placed = call_secret_api(sell_limit_url)
            print
            Fore.GREEN + "Success" if order_placed and order_placed["uuid"] is not None else Fore.RED + "Failed"
            cnt += 1

        print
        order_placed, "Sell #{%s} of #{%s} at #{%s}" % (balance_details["Available"], market_name, sell_price)
    else:
        print
        Fore.RED, "Insufficient Balance"
...
def buy_sell_bot(percent_increase=0.05, chunk=0.004, prepump_buffer=0.5, profit=0.2, splits=2, no_of_retries=10):
    ...
    total_spent = 0.0
    print
    {"low_24_hr": low_24_hr, "last_price": last_price, "ask_price": ask_price}
    if last_price < (1.0 + prepump_buffer) * low_24_hr:  # last_price is smaller than 50% increase since yerterday
        ...
        while counter < no_of_retries:
            ...
            print
            balance_details
            if balance_details and balance_details["Available"] and balance_details[
                "Available"] > 0.0:  # available coins present
                ...
                for i in range(splits):
                    ...
                    sell_price = "%.8f" % sell_price
                    sell_limit_url = get_url(URI_market, action="sell", market=market_name, quantity=qty,
                                             rate=sell_price)
                    print
                    Fore.YELLOW, "Selling coin..."
                    print
                    {"api_type": "market", "action": "sell", "market": market_name, "quantity": qty, "rate": sell_price}
                    order_placed = call_secret_api(sell_limit_url)
                    print
                    Fore.GREEN + "Success" if (
                    order_placed and order_placed["uuid"] is not None) else Fore.RED + "Failed"
                    cnt = 1
                    while cnt <= 3 and order_placed and order_placed["uuid"] is None:  # retry
                        print
                        Fore.YELLOW, "Retry #{cnt} : Selling coin..."
                    ...
                    print
                    order_placed, "Sell #{%s} of #{%s} at #{%s}" % (qty, market_name, sell_price)
                break
            counter += 1
            time.sleep(0.5)
